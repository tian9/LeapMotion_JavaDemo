	...
	public float[][] calFeature(){
		...
		for (int i = 5; i<this.handFrameList.size() - 5; i++){
			...
			for (int j = 0; j < 5; j++){
				...
				float disXPrevious = handFrameList.get(i-1).xAxis[j] -  handFrameList.get(i-2).xAxis[j];
				float disYPrevious = handFrameList.get(i-1).yAxis[j] -  handFrameList.get(i-2).yAxis[j];
				float disZPrevious = handFrameList.get(i-1).zAxis[j] -  handFrameList.get(i-2).zAxis[j];
				...
				if (feature.yxSlopeAngle[j] == Double.NaN){
					feature.yxSlopeAngle[j] = 0;
				}
				if (feature.zxSlopeAngle[j] == Double.NaN){
					feature.zxSlopeAngle[j] = 0;
				}
				...
				if (feature.threeDimAngle[j] == Double.NaN){
					feature.threeDimAngle[j] = 0;
				}
				...
				if (feature.logCurv[j] == Double.NaN){
					feature.logCurv[j] = 0;
				}
